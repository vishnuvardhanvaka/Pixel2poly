...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size, angl_step=4, n_light=1, pitch=[0]):
    ...
    mesh_file = os.path.join(folder_name, subject_name)
    ...
    if not os.path.exists(face_prt_file):
        ...
        return
    just_file_name=subject_name[:subject_name.rfind('_')]
    text_file = os.path.join(folder_name, 'tex', just_file_name + '_dif_2k.jpg')
    ...
    for p in pitch:
        for y in tqdm(range(0, 360, angl_step)):
            ...
            if up_axis == 2:
                R = np.matmul(R, make_rotate(0, math.radians(90), 0).T)
...
if __name__ == '__main__':
    shs = np.load('/content/PIFu/env_sh.npy')
    ...
    subject_name = os.path.basename(args.input)
    base_folder = os.path.dirname(args.input)
    render_prt_ortho(args.out_dir, base_folder, subject_name, shs, rndr, rndr_uv, args.size, 1, 1, pitch=[0])